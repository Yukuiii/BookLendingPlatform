...
const pageSubtitle = computed(() => {
  ...
  return `欢迎回来，${name}。这里先按你给的参考图做成左侧菜单式首页布局。`
})
...
const sideRecommendations = computed(() => {
  return [...books.value]
    .sort((previousBook, nextBook) => (nextBook.borrowCount || 0) - (previousBook.borrowCount || 0))
    .slice(0, 3)
})
...
function handleMenuSelect(menuKey) {
  ...
  if (menuKey !== 'books') {
    ElMessage.info('该功能页还在继续开发中，当前先展示图书检索首页布局')
  }
}
...
<template>
  <main class="home-shell">
    <ConsoleLayout
      ...
      :recommendations="sideRecommendations"
